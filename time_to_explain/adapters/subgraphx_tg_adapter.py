# time_to_explain/adapters/subgraphx_tg_adapter.py
...
# Your existing SubgraphX‑TG + navigators
from ..explainer.tgnnexplainer.subgraphx_tg import SubgraphXTG
from ..explainer.tgnnexplainer.navigators import PGNavigator, MLPNavigator, DotProductNavigator


@dataclass
class SubgraphXTGAdapterConfig:
...
class SubgraphXTGAdapter(BaseExplainer):
    """
    Wraps your SubgraphXTG inside the unified API.
    ...
    def __init__(self, cfg: SubgraphXTGAdapterConfig) -> None:
        super().__init__(name="subgraphx_tg", alias=cfg.alias or f"subgraphx_tg_{cfg.model_name}")
        self.cfg = cfg
        self._explainer: Optional[SubgraphXTG] = None
        self._events = None
    ...
    def prepare(self, *, model: Any, dataset: Any) -> None:
        ...
        # Build your real SubgraphX‑TG (matches your Hydra pipeline)
        self._explainer = SubgraphXTG(
            model=model,
            model_name=self.cfg.model_name,
    ...
    def explain(self, context: ExplanationContext) -> ExplanationResult:
        ...
        if eidx is None:
            raise ValueError("SubgraphXTGAdapter expects an event index: target['event_idx'|'index'|'idx'].")
        ...
        extras: Dict[str, Any] = {
            ...
            "elapsed_sec_subgraphx": raw_pack["elapsed_sec"],
        }
