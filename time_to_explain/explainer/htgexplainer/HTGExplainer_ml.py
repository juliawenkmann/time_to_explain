...
from time_to_explain.utils.pytorchtools import EarlyStopping
from BaseExplainer import HTGNNExplainer
...
class HTGExplainer():
    ...
    def evaluate(self, exp, G_val, G_val_label, t):
        ...
        for i in range(len(G_val)):
            ...
            for n in edge_list:
                ...
                sampling_weights = self.exp(sg, embed, src, dst, self.etype_dic, self.device,'ml')
    ...
    def train(self):
        ...
        model_out_path = 'ckpt/explainer_ml'
        if not os.path.exists(model_out_path):
            os.makedirs(model_out_path)
        early_stopping = EarlyStopping(patience=self.es_epoch, verbose=True, path=f'{model_out_path}/checkpoint_ml_{self.te}_{self.he}.pt')
            ...
            for i in range(len(self.G_train)):
                ...
                for j in range(5):
                    ...
                    for n in training_list:
                        ...
                        # generate the embeddings and predicts
        ...
        if self.test_only:
            ...
            # load the model
            model_out_path = f'ckpt/explainer_ml'
            self.exp.load_state_dict(torch.load(f'{model_out_path}/checkpoint_ml_{self.te}_{self.he}.pt'))
            self.exp.eval()
            all_pred = []
            target_list = []
