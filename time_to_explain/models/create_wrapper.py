...
from time_to_explain.models.device import resolve_device
...
def create_ttgnn_wrapper(
    *,
    ...
    dev = resolve_device(device, cuda=cuda)
    dev_str = dev.type
    model_key = model_type.upper()

    if model_key in ('TGN', 'TTGN'):
        tgn = TTGN(
...
def create_wrapper(
    *,
    ...
    elif model_type.upper() == "TTGN":
        return create_ttgnn_wrapper(
...
def create_tgnn_wrapper(
    *,
    ...
    checkpoint_path: str | None = None,
    parameters: Optional[TrainTestDatasetParameters] = None,
) -> TGNWrapper | TGATWrapper | TTGNWrapper:
    """
    Notebook/utility-friendly dispatcher that mirrors the CLI behavior.
    """
