...
from tgnnexplainer import PROCESSED_DATA_DIR
...
parser.add_argument(
    "--n_degree", type=int, default=50, help="number of neighbors to sample"
)
...
parser.add_argument(
    "--node_dim", type=int, default=None, help="Dimentions of the node embedding"
)
parser.add_argument(
    "--time_dim", type=int, default=None, help="Dimentions of the time embedding"
)
parser.add_argument(
    "--agg_method",
    type=str,
...
g_df = pd.read_csv(PROCESSED_DATA_DIR / "ml_{}.csv".format(DATA))
e_feat = np.load(PROCESSED_DATA_DIR / "ml_{}.npy".format(DATA))
n_feat = np.load(PROCESSED_DATA_DIR / "ml_{}_node.npy".format(DATA))
...
for epoch in tqdm(range(args.n_epoch)):
    ...
    train_auc, train_loss = eval_epoch(
        train_src_l, train_dst_l, train_ts_l, train_label_l, BATCH_SIZE, lr_model, tgan
    )
    test_auc, test_loss = eval_epoch(
        test_src_l, test_dst_l, test_ts_l, test_label_l, BATCH_SIZE, lr_model, tgan
    )
