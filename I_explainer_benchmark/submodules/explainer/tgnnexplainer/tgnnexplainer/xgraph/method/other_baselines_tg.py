...
def _feature_width(value) -> int:
    """Return the trailing feature width for tensors or embedding-like modules."""
    if value is None:
        return 0
    ...
    return int(getattr(value, "embedding_dimension", 0) or 0)
...
class PGExplainerExt(BaseExplainerTG):
    def __init__(self,
        ...
        if self.explainer_ckpt_path.exists():
            # Checkpoints may have been saved on MPS; always remap to CPU first.
            state_dict = torch.load(self.explainer_ckpt_path, map_location="cpu")
            self.explainer_model.load_state_dict(state_dict)
            print(f'explainer ckpt loaded from {str(self.explainer_ckpt_path)}', flush=True)
        else:
            print(f'explainer ckpt not found at {str(self.explainer_ckpt_path)}', flush=True)
            print(f'start training for {self.train_epochs} epochs on {self.device}...', flush=True)
            self._train()
    ...
    def _create_explainer(model, model_name, device):
        if model_name == 'tgat':
            node_dim = _feature_width(getattr(model, "node_raw_embed", None))
            edge_dim = _feature_width(getattr(model, "edge_raw_embed", None))
            time_dim = int(getattr(getattr(model, "time_encoder", None), "dimension", 0) or 0)
            expl_input_dim = 2 * (2 * node_dim + time_dim + edge_dim)
        elif model_name == 'tgn':
            node_dim = _feature_width(getattr(model, "node_raw_embed", None))
            edge_dim = _feature_width(getattr(model, "edge_raw_embed", None))
            time_dim = int(getattr(getattr(model, "time_encoder", None), "dimension", 0) or 0)
    ...
    def _obtain_train_idxs(self,):
        ...
        if self.dataset_name in ['wikipedia', 'reddit']:
            ...
            replace = len(pos_events) < size
            train_e_idxs = np.random.choice(pos_events, size=size, replace=replace)
        else:
            if "e_idx" in self.all_events.columns:
                pool = self.all_events.e_idx.values
            elif "idx" in self.all_events.columns:
                pool = self.all_events.idx.values
            else:
                pool = np.arange(1, len(self.all_events) + 1)

            replace = len(pool) < size
            train_e_idxs = np.random.choice(pool, size=size, replace=replace)
    ...
    def _train(self,):
        ...
        for e in range(self.train_epochs):
            ...
            print(f'epoch {e + 1}/{self.train_epochs} started', flush=True)
            ...
            for i, event_idx in tqdm(
                enumerate(train_e_idxs),
                total=len(train_e_idxs),
                desc=f'epoch {e + 1}/{self.train_epochs}',
            ): # training
            ...
            print(
                f"epoch {e + 1}/{self.train_epochs} loss {mean_loss}, "
                f"skipped: {skipped_num}, ckpt saved: {self.explainer_ckpt_path}",
                flush=True,
            )
        ...
        print(f'explainer ckpt saved at {str(self.explainer_ckpt_path)}', flush=True)
