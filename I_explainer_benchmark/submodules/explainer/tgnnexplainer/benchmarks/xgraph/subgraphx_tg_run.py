...
from tgnnexplainer import RESOURCES_DIR
...
def pipeline(config: DictConfig):
    ...
    config.datasets.dataset_path = str(
        RESOURCES_DIR / "datasets" / "raw" / f"{config.datasets.dataset_name}.csv"
    )
    ...
    config.explainers.results_dir = str(RESOURCES_DIR / "results" / "tgnnexplainer")
    config.explainers.mcts_saved_dir = str(RESOURCES_DIR / "results" / "tgnnexplainer" / "mcts_saved_dir")
    config.explainers.explainer_ckpt_dir = str(RESOURCES_DIR / "explainer" / "tgnnexplainer")
