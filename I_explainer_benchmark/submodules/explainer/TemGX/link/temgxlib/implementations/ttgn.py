...
from temgxlib.utils import ProgressBar
from temgxlib.implementations.connector import TGNNWrapper
from temgxlib.data import ContinuousTimeDynamicGraphDataset, BatchData
from temgxlib.constants import COL_ID

try:
    from TTGN.model.tgn import TGN as TTGNModel
    from TTGN.utils.data_processing import compute_time_statistics
    from TTGN.utils.utils import NeighborFinder
    _TTGN_AVAILABLE = True
except ModuleNotFoundError:
    TTGNModel = None
    compute_time_statistics = None
    NeighborFinder = None
    _TTGN_AVAILABLE = False


def _require_ttgn():
    if not _TTGN_AVAILABLE:
        raise ModuleNotFoundError(
            "TTGN is not available in this repo. "
            "TemGX only supports TGN/TGAT here."
        )


# Implementation of the adjusted TGN model used by Xia et al. 2023 (https://openreview.net/forum?id=BR_ZhvcYbGJ)

def find_candidate_events(dataset: ContinuousTimeDynamicGraphDataset, neighborhood_finder: Any,
...
class TTGNWrapper(TGNNWrapper):
    ...
    def __init__(self, model: Any, dataset: ContinuousTimeDynamicGraphDataset, num_hops: int, model_name: str,
