...
def _resolve_submodules_root() -> Path:
    repo_root = Path(__file__).resolve().parents[3]
    moved = repo_root / "I_explainer_benchmark" / "submodules"
    legacy = repo_root / "submodules"
    return moved if moved.exists() else legacy
...
def _load_temgx_module() -> Any:
    ...
    submodules_root = _resolve_submodules_root()
    root = submodules_root / "explainer" / "TemGX" / "link"
    ...
    tgn_root = submodules_root / "models"
...
class TemGXAdapter(BaseExplainer):
    ...
    def explain(self, context: ExplanationContext) -> ExplanationResult:
        ...
        # Some event neighborhoods can be extremely small under TemGX's temporal index.
        # Backfill from extractor-provided candidates so attribution/mask checks remain meaningful.
        if len(candidate_pool) < 2 and context.subgraph and context.subgraph.payload:
            fallback_raw = context.subgraph.payload.get("candidate_eidx") or []
            target_ts = self._event_ts[eidx]
            existing = set(candidate_pool)
            for e in fallback_raw:
                eid = int(e)
                if eid == eidx or eid in existing:
                    continue
                if eid not in self._event_ts or self._event_ts[eid] >= target_ts:
                    continue
                candidate_pool.append(eid)
                existing.add(eid)
        ...
        score_map_raw = {int(e): float(s) for e, s in scores}
        selected_set = {int(e) for e in selected_edges}
        # Prioritize the model-chosen counterfactual set while retaining ICM ordering
        # among the remaining candidates.
        max_abs_score = max((abs(v) for v in score_map_raw.values()), default=1.0)
        selected_boost = max(1.0, max_abs_score + 1.0)
        score_map = {
            int(e): float(score_map_raw.get(int(e), 0.0) + (selected_boost if int(e) in selected_set else 0.0))
    ...
    def _pack_to_result(
        ...
        p_expl_only = pack.get("counterfactual_prediction")
        extras = {
            "event_idx": eidx,
            ...
            # TemGX's returned score is the prediction when only explanation edges
            # are rolled out (keep-selected), not drop-selected.
            "counterfactual_prediction": p_expl_only,
            "keep_prediction": p_expl_only,
            "prediction_explanation_events_only": p_expl_only,
            "counterfactual_prediction_semantics": "explanation_only_keep_selected",
            "original_prediction": stats.get("original_prediction"),
            "temgx_stats": stats,
        }
