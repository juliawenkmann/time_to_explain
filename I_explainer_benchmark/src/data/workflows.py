...
from . import synthetic_recipes as _synthetic_recipes  # noqa: F401

from ..core.cli import normalize_benchmark_root
from ..core.registry import available_datasets
from ..core.types import DatasetBundle
from .synthetic import prepare_dataset as prepare_synthetic_dataset
from .io import ROOT_DIR, load_processed_dataset
from .tgnn_setup import setup_tgnn_data
from .validate import basic_stats
...
def _resolve_root(root_dir: Optional[Path] = None) -> Path:
    return Path(root_dir).resolve() if root_dir is not None else ROOT_DIR
...
def list_processed_datasets(
    *, root_dir: Optional[Path] = None, include_flat: bool = True
    ...
    root = normalize_benchmark_root(_resolve_root(root_dir), marker="I_explainer_benchmark")
...
def _load_flat_processed_dataset(dataset_name: str, root: Path) -> Optional[DatasetBundle]:
    processed_root = normalize_benchmark_root(root, marker="I_explainer_benchmark") / "resources" / "datasets" / "processed"
    ml_csv = processed_root / f"ml_{dataset_name}.csv"
...
def load_processed_dataset_safe(
    ...
    data_root = normalize_benchmark_root(root, marker="I_explainer_benchmark")
    ...
    try:
        return load_processed_dataset(resolved_name, root_dir=data_root)
        ...
        ensured = ensure_ml_format(resolved_name, root_dir=data_root, verbose=verbose)
...
def prepare_dataset_bundle(
    ...
    *,
    ...
    processed_root = data_root / "resources" / "datasets" / "processed"
    ...
    if chosen_recipe:
        ...
        if cache_hit:
            bundle = load_processed_dataset_safe(dataset_name, root_dir=data_root, verbose=verbose)
