...
class TemporalGNNModelAdapter(ModelProtocol):
    ...
    def _build_preserve_list(
        ...
        *,
        ...
        selected = [int(e) for e, m in zip(candidate_ids, edge_mask) if m and m > 0]

        # Keep non-candidate support edges visible when masking candidates.
        support_raw = subgraph.payload.get("support_eidx")
        base_raw = subgraph.payload.get("base_eidx")

        if support_raw is not None:
            support = [int(e) for e in support_raw]
            candidate_set = {int(e) for e in candidate_ids}
            base = [int(e) for e in support if int(e) not in candidate_set]
        elif base_raw is not None:
            base = [int(e) for e in base_raw]
        else:
            base = []

        preserve = sorted(set(base).union(selected))
        return preserve
