...
_TEMGX_VENDOR = _SUBMODULES_ROOT / "explainer" / "TemGX" / "link"
if str(_TEMGX_VENDOR) not in sys.path:
    sys.path.insert(0, str(_TEMGX_VENDOR))

_TGN_MODELS_ROOT = _SUBMODULES_ROOT / "models"
_TGN_VENDOR_ROOT = _TGN_MODELS_ROOT / "tgn"
for p in (_TGN_MODELS_ROOT, _TGN_VENDOR_ROOT):
    if str(p) not in sys.path:
        sys.path.insert(0, str(p))

from temgxlib.data import BatchData, ContinuousTimeDynamicGraphDataset
try:
    from submodules.models.tgn.utils.data_processing import Data, compute_time_statistics
    from submodules.models.tgn.utils.utils import get_neighbor_finder
except ModuleNotFoundError:
    from tgn.utils.data_processing import Data, compute_time_statistics
...
class TGNNWrapper:
    ...
    def _supports_temporal_embeddings(self) -> bool:
        return callable(getattr(self.model, "compute_temporal_embeddings", None))

    def _predict_with_get_prob(
        self,
        ...
        *,
        ...
        if edge_idx_preserve_list is not None:
            kwargs["edge_idx_preserve_list"] = [int(e) for e in edge_idx_preserve_list]
        if candidate_weights_dict is not None:
            kwargs["candidate_weights_dict"] = candidate_weights_dict

        pos = self.model.get_prob(source_nodes, target_nodes, timestamps, **kwargs)
    ...
    def _compute_scores(
        ...
        *,
        ...
        try:
            grad_context = torch.no_grad() if self.evaluation_mode else contextlib.nullcontext()
            with grad_context:
    ...
    def predict(
        ...
        if candidate_events is not None and edge_weights is not None:
            ...
            pos = self._predict_with_get_prob(
                src,
                dst,
                ts,
        ...
        if not self._supports_temporal_embeddings():
            pos = self._predict_with_get_prob(
                src,
                dst,
                ts,
    ...
    def compute_embeddings(
        ...
        if not self._supports_temporal_embeddings():
            raise NotImplementedError(
                "compute_embeddings requires model.compute_temporal_embeddings (not available for this backbone)."
            )
    ...
    def compute_edge_probabilities_for_subgraph(
        ...
        if not callable(getattr(self.model, "set_neighbor_finder", None)):
            if event_ids_to_rollout is None:
                preserve_dataset_ids = self._edge_ids[~np.isin(self._edge_ids, edges_to_drop)]
            else:
                preserve_dataset_ids = self._as_numpy(event_ids_to_rollout)
            preserve_dataset_ids = preserve_dataset_ids[preserve_dataset_ids < event_id]
            preserve_model_ids = self._to_model_event_ids(preserve_dataset_ids).tolist()
            ...
            pos = self._predict_with_get_prob(
                ...
                dst,
                ts,
                edge_idx_preserve_list=preserve_model_ids,
                result_as_logit=result_as_logit,
            )
            return pos, None
