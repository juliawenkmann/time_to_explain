...
_CONFIG_PATH = Path(__file__).resolve().parents[3] / "configs" / "global" / "plots.json"
...
_DEFAULT_PLOT_DIR = ASSET_ROOT / "resources" / "results" / "plots"
_NOISY_IMAGE_EXPORT_LOGGERS = (
    "choreographer",
...
_IMAGE_EXPORT_LOGGING_SUPPRESSED = False
...
def _suppress_image_export_info_logs() -> None:
    global _IMAGE_EXPORT_LOGGING_SUPPRESSED
    if _IMAGE_EXPORT_LOGGING_SUPPRESSED:
        return
    for logger_name in _NOISY_IMAGE_EXPORT_LOGGERS:
        logging.getLogger(logger_name).setLevel(logging.WARNING)
    _IMAGE_EXPORT_LOGGING_SUPPRESSED = True
...
def _maybe_save(fig: "go.Figure", save_to: Union[str, Path, bool, None]) -> str:
    ...
    if save_to.lower().endswith(".html"):
        ...
        _suppress_image_export_info_logs()
